...
vec4 computeIncidentLight(vec3 _pos,int planetIndex, float fMieControl){
    ...
    float earthRadius=695.0;
    float atmosphereRadius = 1000.0;
    vec3 v3CameraPos = uCameraPosition;
    ...
    for (int i = 0; i < numSamples; ++i) 
    {
        ...
        float height = length(samplePosition - uPlanetPosition) - earthRadius;
        float hm = denseFunc(-height / Hm) * segmentLength;
        opticalDepthM += hm;
        tCurrent += segmentLength;
    }
    vec3 color = pow(opticalDepthM,0.1)*orange2;
