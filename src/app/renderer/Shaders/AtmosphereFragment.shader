...
float dPlanetSize = 10.0;
vec4 computeIncidentLight(vec3 _pos,int planetIndex, float fMieControl){
    float tmin=0.0;
    float tmax=1000000000.0;
    float earthRadius=uPlanetSize;// - 0.1 * uPlanetSize;// -9.93;
    float atmosphereRadius = uPlanetSize +1.0;//11.0;
    //_pos-=uPlanetPosition;
    vec3 v3CameraPos = uCameraPosition;//-uPlanetPosition;
    vec3 orig = v3CameraPos;
    vec3 dir=normalize(_pos - v3CameraPos);
    vec3 v3LightPos = uLightPosition;
    vec3 sunDirection = normalize(v3LightPos-_pos);
    ...
    float dist_2 = float(length(_pos-v3CameraPos) - tmin) / float(numSamples);
    ...
    float Hr = uRayleigh.w;
    float Hm = uMie.w;
    float power = -1.0;
    vec3 betaM = uMie.xyz;//vec3(5e-6f); 
    vec3 betaR = uRayleigh.xyz;//vec3(1.5*pow(10.0,power), 4.0*pow(10.0,power), 8.*pow(10.0,power));
    float g = 0.9999;
    ...
    vec3 color = (sumR * betaR * phaseR + fMieControl*sumM * betaM * phaseM) * fLightPower;
...
void main(){
  ...
  if(v4AtmosphereLayer.w != 0.0)
  {
    ...
    float fDistToCurrFragment=distance(uCameraPosition, position.xyz);
    ...
    {
      FinalColor.xyz=computeIncidentLight(v4AtmosphereLayer.xyz,planetIndex,1.0).xyz;
      return;
    }
    return;
  }
