...
void main(void) {
    float timeSpeed=1.1;
    vec4 positionModelSpace=inverse(uModelMatrix)*vec4(Position_worldspace,1);
    float x=10.0;
    vec3 initial=vec3(x,x,x);
    vec4 color;
    vec3 centre = (uModelMatrix*vec4(0.0,0.0,0.0,1.0)).xyz;
    color = yellow*(1.0*cnoise(vec4(initial,1.0)));
    // Get the distance vector from the center
    vec3 nDistVec = normalize(fPosition-centre);
    float A=0.1;
    float f=10.0;
    // Get noise with normalized position to offset the original position
    vec3 position = fPosition + nDistVec*(A*cnoise(vec4(f*nDistVec, uTime))) ;

    color.w           = 1.0;
    float dist        = distance(position,centre)* 1.0;
    float brightness  = (1.0 / pow(dist,3.5) - 0.5) * 0.05;
    color             = vec4(orange.xyz,1.0)*brightness;
    color.a           = brightness;
    ...
    OutputColor=color;
    OutputColorProperties.xyz=vec3(0.0,1.0,1.0);
