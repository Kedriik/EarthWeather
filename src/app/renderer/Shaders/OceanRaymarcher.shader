...
vec3 rayDirection(float fieldOfView, vec2 size, vec2 fragCoord) {
  ...
  float sphere( in vec3 p){
    ...
    spherePos=uViewMatrix*spherePos;
  ...
  vec3 materialProp;
  vec3 ocean_color=vec3(0,119.0/255.0,190.0/255.0);
  float ocean(in vec3 p){
    ...
    float A=0.028;
    float f=100.0;
    float R=uPlanetSize + 0.01*uPlanetSize;
    float dist=planetarySphere(p,R);
    if(dist>0.2)
    {
      return dist;
    }
    ...
    dist=distance(p,spherePos.xyz) - R;//(R+A*terrain(vec4(f*seed.xyz,0)));
...
void main(void) {
  ...
  if(abs(sceneSDF(p)) < epsilon){
    ...
    OutputColor.w     = 1.0;
    OutputColorProperties.xyz = vec3(1,0.5,0);
