...
float RidgedMultifractal( vec3 point,float A, float freq){
 ...
 // 30,3, 8, 2, 0.25
  ...
  float terrain(vec4 in_seed){
    ...
    float f=0.01, A=200.1;
    float RMA=1.0, RMF=1.0;
    ...
    if(h>0.0){
      ...
      RMF=0.04;
      A=10.1;
      //float terrainProp1=clamp(A*snoise(f*seed),0.0,A);
      float threshold = 0.01;
      float powFactor=2.2;
      //h=RidgedMultifractal(seed,RMA,RMF);
      h+=1.0*(pow(RidgedMultifractal(seed,RMA*terrainProp,RMF),powFactor))/1.0;
      ...
      f=0.01;
     //h+=fbm_1(f*seed,A);
     //TODO: NEED SMOOTH FIX !
     //h-=0.5;
    }
    return h;
}
vec3 snow=vec3(1.0,1.0,1.0 );
vec3 ice=vec3(104.0/255.0,123.0/255.0,243.0/255.0);
vec3 rock=vec3(81.0/255.0,84.0/255.0,102.0/255.0);
vec3 gravel=vec3(80.0/255.0, 45.0/255.0, 28.0/255.0);
vec3 sand=vec3(234.0/255.0, 164.0/255.0, 0);
vec3 grass=vec3(0.486, 0.988,0.0);
vec3 redRock1=vec3(45.0/225.0,11.0/225.0, 11.0/255.0);
vec3 redRock2=vec3(156.0/225.0,9.0/225.0, 9.0/255.0);
vec3 moonColor=vec3(212.0/255.0, 170.0/255.0, 144.0/255.0);
...
vec3 rayDirection(float fieldOfView, vec2 size, vec2 fragCoord) {
  ...
  vec2 coordinates2(vec3 dir){
    vec3 prime_meridian = vec3(1.0,0.0,0.0);
    vec3 y = vec3(0.0,1.0,0.0);
    vec3 z = vec3(0.0,0.0,1.0); 
    
    
    float lat = asin(dir.y/uPlanetSize); //theta
    float lon = atan(dir.x / dir.z); //phi
        
    float u = lon / PI;
    float v = PI/2.0 - 2.0*atan(exp(-2.0*PI*lon));
